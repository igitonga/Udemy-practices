...
export default function WelcomePage() {
  ...
  const bgCityOpacity = useTransform(scrollY, [0, 200, 300, 500], [1, 0.5, 0.5, 0]);
  const bgCityYAxis = useTransform(scrollY, [0, 200], [0, -100]);
  const bgText = useTransform(scrollY, [0, 200], [1.1, 1.5]);
  const bgHeroYAxis = useTransform(scrollY, [0, 200], [0, -150]);
    ...
    <>
      <header id="welcome-header">
        <motion.div id="welcome-header-content">
          <motion.h1
            style={{ scale: bgText }}
          >Ready for a challenge?</motion.h1>
          <Link id="cta-link" to="/challenges">
            Get Started
          </Link>
        </motion.div>
        <motion.img
          style={{ opacity: bgCityOpacity, y: bgCityYAxis }}
          src={cityImg}
